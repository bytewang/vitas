					...
					if (attr_value.startsWith("javascript:", false)) {
						...
						switch (replacetype) {
							case imageType: 
								{
									int len;
									CachedImage* cachedImage =  param->doc->docLoader()->requestImage(url.string()); 
									ASSERT(cachedImage);
							...
							case cssType:
								{
									String charset;
									CachedCSSStyleSheet *cachecsssheet =  param->doc->docLoader()->requestUserCSSStyleSheet(url.string(), charset); 
							...
							case jscriptType:
								{
									String charset;
									CachedScript *cacheScirpt = param->doc->docLoader()->requestScript(url.string(), charset); 
							...
							case iframeType:
								{
									...
									if ((is_frame_element && web_frame)) {
										...
										for (j = 0; j < frames_.size(); j++ ) {
											if ( web_frame->loader()->url() == url.string())
												break;
										}
		...
		SerializeDomParam* param)
{
	...
	if (element->hasChildNodes() || param->has_added_contents_before_end) {
		...
		if (param->is_html_document) {
			...
			if (html_element->endTagRequirement() == TagStatusRequired) {
...
void DomSerializer::StartDownloadSubresourceFromURL(const KURL &url, const String & file)
{
	...
	RefPtr<ResourceHandle> handle = ResourceHandle::create(request, new DownloadResourceClient(file), 0, false, false);
	return;
}
...
{
	KURL main_page_kurl(m_mainframe->loader()->url());
	...
	for (int i = 0; i < static_cast<int>(frames_.size()); ++i) {
		...
		const KURL& current_frame_kurl = KURL(current_frame->loader()->url());
