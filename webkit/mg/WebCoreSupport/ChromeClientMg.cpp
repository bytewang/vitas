...
Page* ChromeClientMg::createWindow(Frame* frame, const FrameLoadRequest& request, const WindowFeatures& features)
{
...
void ChromeClientMg::setToolTip(const WebCore::String& text, WebCore::TextDirection direction)
{
    IMDWebUIDelegate *uiDelegate = m_webView->uiDelegate();
    ASSERT(uiDelegate);
...
void ChromeClientMg::runOpenPanel(Frame*, PassRefPtr<FileChooser> fileChooser)
{
    ...
    if(m_webView->uiDelegate()->chooseFiles(m_webView, &fileInfo))
    {
        ...
        for(int i= 0; i < fileInfo.files_count; i++)
            fileList.append(fileInfo.files[i]);
...
bool ChromeClientMg::setCursor(PlatformCursorHandle)
{
    notImplemented();
    return false;
}
