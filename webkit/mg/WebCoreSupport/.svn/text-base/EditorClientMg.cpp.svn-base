...
void EditorClientMg::setInputMethodState(bool enabled)
{
    ...
    if (frame && frame->document() && frame->document()->focusedNode()) {
        ...
        // The IME was not allowed for password filed in WebCore, however, we need it.
        if (!enabled && node->hasTagName(HTMLNames::inputTag)) {
            HTMLInputElement* inputElement = static_cast<HTMLInputElement*>(frame->document()->focusedNode());
            enabled = inputElement->isPasswordField();
        }
#if ENABLE(WML)
        else if (!enabled && node->hasTagName(WMLNames::inputTag)) {
            WMLInputElement* inputElement = static_cast<WMLInputElement*>(frame->document()->focusedNode());
            enabled = inputElement->isPasswordField();
        }
#endif
    }
...
void EditorClientMg::getGuessesForWord(const String&, Vector<String>& guesses)
{
    notImplemented();
}
}
