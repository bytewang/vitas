...
void FrameLoaderClientMg::dispatchDecidePolicyForMIMEType(FramePolicyFunction policyFunction, 
        const String& MIMEType, const ResourceRequest&req)
{
    ...
    result = policyDelegate->decidePolicyForMIMEType(webView, MIMEType.utf8().data(), (MDHRequest)&req, m_webFrame);
...
static String composeUserAgent()
{
    ...
    os = agentOS() + ";";
    lang += String("en") + "; "; // Localization information FIXME: not implement Use "en" for now
    ...
    mdolphin+=".";
    mdolphin +=  String::number(MDOLPHIN_MICRO_VERSION);
    ...
    webkit+=".";
    webkit +=  String::number(WEBKIT_MICRO_VERSION);
    ...
    safria+=".";
    safria +=  String::number(WEBKIT_MICRO_VERSION);
#if 0
    // Product
    ua += "Mozilla/5.0 ";

    // Comment
    ua += "(MiniGUI/3.X; "; // Platform
    ua += "U; ";  // Security
    ...
    // FIXME: The WebKit version is hardcoded
    ua += "mDolphin/2.00 ";
    ua += "(Compatible Gecko/20070601, Firefox/1.5.0)";

    /* When I signup Hotmail, the User-Agent should be facked, e.g
     * "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.1.12) Gecko/20080314 BonEcho/2.0.0.12";
     */
#else
    // Product
    ua += "Mozilla/5.0";
...
void FrameLoaderClientMg::dispatchDidReceiveTitle(const String& title)
{
    MDWebView* webView = m_webFrame->topLevelView();
    char* str = copyStringToUtf8Char(title);
...
void FrameLoaderClientMg::setTitle(const String& title, const KURL& url)
{
    // no need for, dispatchDidReceiveTitle is the right callback
}
...
Frame* FrameLoaderClientMg::dispatchCreatePage()
{
...
ObjectContentType FrameLoaderClientMg::objectContentType(const KURL& url, const String& _mimeType)
{
    if (url.isEmpty())
