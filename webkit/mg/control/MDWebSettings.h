/*
 ** $Id: MDWebSettings.h 528 2011-03-18 08:10:33Z xwyan $
 **
 ** MDWebSettings.h: 
 **
...
#define DBL_PROP_DEFINE(Getter, Setter)  TYPE_PROP_DEFINE(double, Getter, Setter)
    ...
    INT_PROP_DEFINE(localStorageQuota, setLocalStorageQuota)
...
protected:
    ...
    int setStringValue(const WebCore::AtomicString& str, char* buffer, int max) const{

        if(!buffer || max <= 0)
            return  -1;
        const WebCore::String & s = (const WebCore::String&)str;
...
////meta property
    ...
    public:
        ...
        const WebCore::String & name() const { return _name; }
        void setName(const WebCore::String& name) { _name = name; }
    ...
    static WTF::HashMap<WebCore::String, PropertyMeta*> _properties;
