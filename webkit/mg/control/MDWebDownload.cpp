...
void DownloadClient::didFinishLoading(ResourceHandle* handle)
{
...
void MDWebDownload::start()
{
    ...
    if (!d->resourceHandle)
        d->resourceHandle = ResourceHandle::create(*d->networkRequest, d->downloadClient, 0, false, false);
        ...
        const char* ptr /* 0 */, int length /* 0 */)
{
    ...
    data->url = copyStringToUtf8Char(job->request().url().string());
