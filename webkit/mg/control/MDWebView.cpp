        ...
        switch(item.type()) {
            ...
            case WebCore::ActionType: { 
                ...
                if (action != MD_POPMENU_NO) {
                    coreMenu->checkOrEnableIfNeeded(item); 
...
void MDWebView::initSettings(Settings *settings)
{
    ...
    settings->setSansSerifFontFamily(AtomicString("Arial"));
...
void MDWebView::init(HWND hwnd)
{
    ...
    m_page = new Page(new ChromeClientMg(this), new ContextMenuClientMg(this),
                          new EditorClientMg(this), new DragClientMg, new InspectorClientMg, 0, 0);
    ...
    //init WebBackForward object
    m_backForwardList = MDWebBackForwardList::createInstance(m_page->backForwardList()); 
...
IMDWebView* CreateWebViewInstance(RECT rect, int id, HWND hParent)
{
...
IMDWebView* GetWebViewInstance(HWND hWnd)
{
    IMDWebView*  view = reinterpret_cast<IMDWebView*>(GetWindowAdditionalData2(hWnd));
    return view;
}
