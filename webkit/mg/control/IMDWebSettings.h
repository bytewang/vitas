...
public:
    ...
    virtual void setLocalStorageQuota(unsigned) = 0;
    virtual unsigned localStorageQuota() const  = 0;

    // Allow clients concerned with memory consumption to set a quota on session storage
    // since the memory used won't be released until the Page is destroyed.
    // Default is noQuota.
    virtual void setSessionStorageQuota(unsigned) = 0;
    virtual unsigned sessionStorageQuota() const  = 0;
#endif
